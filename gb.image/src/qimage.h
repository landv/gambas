...
public:
	...
	GB_IMAGE object() { return img; }
	int depth() const { return 32; }
	int width() const { return info.width; }
	int height() const { return info.height; }
	int numColors() const { return 0; }
	QRgb *colorTable()	const { return NULL; }
	QRgb pixel(int x, int y) const { return ((uint *)info.data)[y * width() + x]; }
	void setPixel(int x, int y, QRgb c) { ((uint *)info.data)[y * width() + x] = c; }
	bool inverted() const { return inv; }
	void invert();
	void invertPixels();
	bool transparent() { return GB_IMAGE_TRANSPARENT(info.format); }

	uchar *scanLine( int ) const;
	uchar *bits() const { return (uchar *)info.data; }
  uchar **jumpTable();
