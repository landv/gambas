    ...
    bool Execute(char *query){
      try{
        ...
        if(pe && pe->EngineCode()!=335544321){
          GB.Error("Error to Execute the Query !");
          return FALSE;
      }
      else{
...
static void query_init(DB_RESULT result, DB_INFO *info, long int *count){
...
static int table_index(DB_DATABASE *db, char *table, DB_INFO *info){
  ...
  for (i = 0; i < n; i++){
    for (j = 0; j < info->nfield; j++){
      if(strcasecmp(info->field[j].name, res->GetData(j,1).value._string.value) == 0){
        info->index[i] = j;
        break;
      }
    }
