...
static int gTabStrip_buttonRelease(GtkWidget *wid,GdkEventButton *e,gTabStrip *d)
{
	...
	if (d->onMouseEvent)
	{
		gMouse::setValid(2,(long)e->x,(long)e->y,e->button,e->state,d->screenX(),d->screenY());
...
static int gTabStrip_buttonPress(GtkWidget *wid,GdkEventButton *e,gTabStrip *d)
{
	...
	if (d->onMouseEvent)
	{
		gMouse::setValid(1,(long)e->x,(long)e->y,e->button,e->state,d->screenX(),d->screenY());
...
static void gTabStrip_click(GtkNotebook *nb,GtkNotebookPage *pg,guint pnum,gTabStrip *data)
{
	data->performArrange();
