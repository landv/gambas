...
{
	if (data->onFinish) data->onFinish(data);
	delete data;
}
...
gboolean sg_configure(GtkWidget *widget,GdkEventConfigure *event,gControl *data)
{
	GtkWidget *fr;
	GList *chd;
...
gboolean sg_key_Press (GtkWidget *widget, GdkEventKey *event, gControl *data)
{
	...
	gKey::enable(event);
	if (data->onKeyEvent) vl=data->onKeyEvent(data,gEvent_KeyPress);
	gKey::disable();
	...
	if (!vl)
	{
		if (data->getClass()==Type_gMainWindow)
		{
			if ( (event->keyval==GDK_Escape) && ( ((gMainWindow*)data)->esc_button!=NULL) ) 
				gtk_button_clicked(GTK_BUTTON(((gMainWindow*)data)->esc_button));
		}
	}
...
gboolean sg_button_Press (GtkWidget *widget,GdkEventButton *event,gControl *data)
{
	...
	if (data->onMouseEvent)
	{
		...
		if (event->button==3) data->onMouseEvent(data,gEvent_MouseMenu);
	}
	return false;
}
...
gboolean sg_scroll(GtkWidget *widget,GdkEventScroll *event,gControl *data)
{
	long bt=0;
	long dt=0;
	long ort=0;
...
{
	if (data->pr) ((gContainer*)data->pr)->performArrange();
	if ( data->getClass() & 0x0100 ) ((gContainer*)data)->performArrange();

	if ( data->getClass()==Type_gEmbedWindow)
		if ( ((gMainWindow*)data)->onResize) 
			((gMainWindow*)data)->onResize((gMainWindow*)data);
}
...
{	
	...
	if (pr) g_signal_connect(G_OBJECT(border),"configure-event",G_CALLBACK(sg_configure),(gpointer)this);
	g_signal_connect(G_OBJECT(border),"destroy",G_CALLBACK(sg_destroy),(gpointer)this);
	g_signal_connect(G_OBJECT(border),"key-press-event",G_CALLBACK(sg_key_Press),(gpointer)this);
	g_signal_connect(G_OBJECT(border),"key-release-event",G_CALLBACK(sg_key_Release),(gpointer)this);
