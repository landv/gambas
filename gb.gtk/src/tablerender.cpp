...
void gTableRender::render(GdkRectangle *ar)
{
	...
	if ((bpos-offY) <= visibleHeight())
	{
		//gdk_draw_rectangle(sf->window,gc,TRUE,0,bpos-offY,pmaxX,visibleHeight() - (bpos-offY));
	...
	if ((bpos-offX) <= visibleWidth())
	{
		//gdk_draw_rectangle(sf->window,gc,TRUE,bpos-offX,0,visibleWidth() - (bpos-offX),pmaxY);
