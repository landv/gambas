...
static void send_configure (gControl *control)
{
  ...
  widget = control->border;
	...
	//if (control->isWindow())
	//  fprintf(stderr, "send configure: %p\n", control);
...
void gControl::move(int x, int y)
{
	...
	if (pr && pr->getClass()==Type_gSplitter) return;
...
void gControl::updateCursor(GdkCursor *cursor)
{
  ...
  if (frame && frame != border && GDK_IS_WINDOW(frame->window))
...
void gControl::dragPicture(gPicture *img)
{
	GtkTargetList *list;
	//GdkPixbuf *buf;
	GdkDragContext *ct;
	...
	gControl::freeDragBuffer();
	if (img) {
		drag_picture_buffer=img->getPixbuf();
...
static gboolean cb_frame_expose(GtkWidget *wid, GdkEventExpose *e, gControl *control)
{
	...
	if (control->getFrameBorder() == BORDER_NONE)
    return false;
	...
	//printf("draw frame: %d\n", control->getFrameBorder()); fflush(stdout);
  //gdk_drawable_get_size(win, &w, &h);
  w = control->width();
  h = control->height();
  
	switch (control->getFrameBorder())
	{
    case BORDER_PLAIN:
    {
      GtkStyle* st = gtk_rc_get_style_by_paths(gtk_settings_get_default(), NULL, "GtkEntry", GTK_TYPE_ENTRY);
      
      gc = gdk_gc_new(GDK_DRAWABLE(win));
      gdk_gc_set_foreground(gc, &st->fg[GTK_STATE_NORMAL]);
  ...
  gtk_paint_shadow(wid->style, win, GTK_STATE_NORMAL, shadow, &e->area, NULL, NULL, 0, 0, w, h);
	return false;
}
...
void gControl::realize(bool make_frame)
{
  ...
  else if (!frame)
  {
    frame = gtk_alignment_new(0, 0, 1, 1);
		gtk_widget_set_double_buffered(frame, false);
  }
