...
void gControl::drawBorder(GdkDrawable *win)
{
  ...
  if (w < 1 || h < 1)
    ...
    case BORDER_PLAIN:
    {
      ...
      gdk_gc_set_foreground(gc, use_base ? &st->text[GTK_STATE_NORMAL] : &st->fg[GTK_STATE_NORMAL]);
...
/*	GtkStyle* st = gtk_widget_get_style(control->widget);
	GdkColor color;
	gc = gdk_gc_new(GDK_DRAWABLE(win));
	fill_gdk_color(&color, 0xFF0000);
	gdk_gc_set_foreground(gc, &color); //control->use_base ? &st->base[GTK_STATE_NORMAL] : &st->bg[GTK_STATE_NORMAL]);
	gdk_draw_rectangle(win, gc, FALSE, 0, 0, w - 1, h - 1); 
	gdk_draw_rectangle(win, gc, FALSE, 1, 1, w - 3, h - 3); 
	g_object_unref(gc);*/
