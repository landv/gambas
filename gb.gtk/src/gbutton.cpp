...
static void bt_click(GtkButton *object, gButton *data)
{
	...
	if (data->type == gButton::Tool)
	{
		...
		{
			data->disable = true;
			gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(data->border), false);
		}
	}
...
gButton::gButton(gContainer *par, Type typ) : gControl(par)
{
  ...
  type = typ;
	...
	{	
		...
		gtk_container_add (GTK_CONTAINER(border),GTK_WIDGET(lbl));
	}
	
	realize();
	
	gtk_button_set_focus_on_click (GTK_BUTTON(border),true);
	gtk_widget_add_events(border, GDK_POINTER_MOTION_MASK);
	onClick=NULL;
	
	if (type == Radio)
		g_signal_connect(G_OBJECT(border),"clicked",G_CALLBACK(rd_click),(gpointer)this);
	else
		g_signal_connect(G_OBJECT(border),"clicked",G_CALLBACK(bt_click),(gpointer)this);	
...
void gButton::setInconsistent(bool vl)
{
	...
	gtk_toggle_button_set_inconsistent (GTK_TOGGLE_BUTTON(border),vl);
}
...
bool gButton::inconsistent()
{
	...
	g_object_get (G_OBJECT(border),"inconsistent",&vl,NULL);
	return vl;	
}
...
void gButton::setText(const char *st)
{
	...
	if (!rendtxt)
	{
		lbl=(GtkLabel*)gtk_bin_get_child(GTK_BIN(border));
		if (!st) st="";
		if (!strlen(st))
		{ 
	...
	if (shortcut) 
		gtk_widget_add_accelerator(border, "clicked", accel, (guint)shortcut, GDK_MOD1_MASK, (GtkAccelFlags)0);
...
{
	switch(gtk_button_get_relief(GTK_BUTTON(border)))
	{
...
{
	if (vl)
		gtk_button_set_relief (GTK_BUTTON(border),GTK_RELIEF_NORMAL);
	else
		gtk_button_set_relief (GTK_BUTTON(border),GTK_RELIEF_NONE);
}
...
void gButton::setDefault(bool vl)
{
	...
	{
		win->_default = this;
		GTK_WIDGET_SET_FLAGS (border, GTK_CAN_DEFAULT);
		//gtk_widget_grab_default (widget);
	}
	else
	{
		GTK_WIDGET_UNSET_FLAGS(border, GTK_CAN_DEFAULT);
...
bool gButton::value()
{
  if (type == Button)
    return false;
		...
		return gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON(border));
}
...
void gButton::setValue(bool vl)
{
  if (type == Button)
  {
    if (vl) gtk_button_clicked(GTK_BUTTON(border));
  }
  else
    gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON(border),vl);
}
...
void gButton::animateClick(bool on)
{
	...
	if (!on && !_animated)
	{
		gtk_widget_set_state(border, GTK_STATE_ACTIVE);
		_animated = true;
	}
	else if (on && _animated)
	{
		_animated = false;
		gtk_widget_set_state(border, GTK_STATE_NORMAL);
		gtk_button_clicked(GTK_BUTTON(border));
	}
}
