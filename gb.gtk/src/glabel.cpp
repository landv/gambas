...
gboolean label_exp(GtkWidget *wid,GdkEventExpose *e,gSimpleLabel *d)
{
	...
	if (d->markup)
	{
		width = d->width() - d->getFrameWidth() * 2;
		if (width < 0)
			return false;
		width *= PANGO_SCALE;
	}
	...
	gdk_draw_layout(d->widget->window,gc,vw,vh,d->layout);
	
	g_object_unref(G_OBJECT(gc));
	return false;
}

gSimpleLabel::gSimpleLabel(gContainer *parent) : gControl(parent)
{
	textdata=NULL;
	g_typ=Type_gLabel;
	markup=false;
	...
	g_signal_connect_after(G_OBJECT(widget),"expose-event",G_CALLBACK(label_exp),(gpointer)this);
...
void gSimpleLabel::setText(char *vl)
{
	...
	else
	{
		if (markup)
		{
			bpango = gt_html_to_pango_string(textdata, false);
			if (!bpango)
				pango_layout_set_text(layout,"",-1);
			else
			{
...
void gSimpleLabel::resize(int w, int h)
{
	gint mh;
