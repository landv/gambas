...
static gControl* get_next_child_widget (gContainer *gtk_control,long *gtk_list,long gtk_count)
{
	gControl *ctrl;
...
int gContainer::clientX()
{
	...
	if (!cont->window || !border->window || cont == border)
		return getFrameWidth();
		
	gtk_widget_translate_coordinates(cont, border, 0, 0, &x, &y);
	return x; //+ getFrameWidth();
}
...
int gContainer::clientWidth()
{
	...
	else if (GTK_IS_SCROLLED_WINDOW(border))
