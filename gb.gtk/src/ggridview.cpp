	...
	switch(data->selectionMode())
	{
		...
		case SELECT_MULTIPLE:
			...
			else
			{
				...
				{
					for (bc = bcurrent; bc <= data->sel_row; bc++)
						data->setRowSelected(bc, true);
				}
				else
				{
					for (bc = data->sel_row; bc <= bcurrent; bc++)
						data->setRowSelected(bc, true);
				}
...
static gboolean tblateral_press(GtkWidget *wid,GdkEventButton *e,gGridView *data)
{
	...
	if (e->button != 1 || data->findRowSeparation(pos) >= 0)
		return FALSE;
...
static gboolean cb_contents_button_press(GtkWidget *wid, GdkEventButton *e, gGridView *data)
{
	...
	if (e->type != GDK_BUTTON_PRESS && e->type != GDK_2BUTTON_PRESS) 
		return FALSE;

	if (e->button != 1) 
		return FALSE;

	row = data->render->findVisibleRow((int)e->y);
	col = data->render->findVisibleColumn((int)e->x);
	
	data->setCursor(row, col);
	
	if (e->type == GDK_BUTTON_PRESS)	
		{ if (data->onClick) data->onClick(data, row, col); }
	else
...
static gboolean gridview_expose(GtkWidget *wid,GdkEventExpose *e,gGridView *data)
{	
	gint w,h;
...
gGridView::gGridView(gContainer *parent) : gControl(parent)
{
	...
	g_signal_connect(G_OBJECT(contents),"event",G_CALLBACK(cb_contents_button_press),this);
...
void gGridView::setItemText(int row,int col,char *vl)
{
...
void gGridView::setHeaderText(int col,char *value)
{
	gGridHeader *head;
	int w;
...
void gGridView::setFooterText(int col,char *value)
{
	gGridHeader *head;
...
void gGridView::setRowText(int col,char *value)
{
	gGridRow *head;
