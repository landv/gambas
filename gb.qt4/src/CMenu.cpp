...
static void hide_menu(CMENU *item)
{
  ...
  CONTAINER_CALL(item, removeItem(item->id));

  if (CMENU_is_top(item))
  {
    if (CONTAINER_FUNC(item, count()) == 0)
    {
      item->parentMenuBar->hide();
...
static void show_menu(CMENU *item)
{
  ...
  if (parent)
  {
    item->parentMenu = QMENU(parent);
  }


  for (pos = 0; pos < (int)CONTAINER_FUNC(item, count()); pos++)
  {
    if (CONTAINER_FUNC(item, idAt(pos)) >= item->id)
      break;
  }
  ...
  if (CMENU_is_separator(item))
  {
    ...
    //CONTAINER_CALL(item, insertItem((QWidget *)0, item->id, pos));
    item->id = CONTAINER_FUNC(item, insertSeparator(pos));
  }
  else if (QMENU(item) != 0)
  {
#ifdef DEBUG_MENU
    qDebug("show_menu: insertItem('%s', %p, %d, %d)", item->text, QMENU(item), item->id, pos);
#endif
    if (icon.isNull())
      CONTAINER_CALL(item, insertItem(text, QMENU(item), item->id, pos))
    else
      CONTAINER_CALL(item, insertItem(icon, text, QMENU(item), item->id, pos))
  }
  else
  {
#ifdef DEBUG_MENU
...
#endif
    if (icon.isNull())
      CONTAINER_CALL(item, insertItem(text, item->id, pos))
    else
      CONTAINER_CALL(item, insertItem(icon, text, item->id, pos))
  }
  ...
  CONTAINER_CALL(item, setItemEnabled(item->id, item->enabled));
  CONTAINER_CALL(item, setItemChecked(item->id, item->checked));

  if (CMENU_is_top(item))
  {
    if (item->parentMenuBar->count() == 1)
		{
			item->parentMenuBar->show();
			((MyMainWindow *)item->toplevel)->configure();
		}
	}
...
static void init_menu(CMENU *item)
{
  ...
  if (parent)
  {
    if (QMENU(parent) == 0)
    {
      ...
      QMenu *popup = new QMenu(item->toplevel);
...
BEGIN_METHOD(CMENU_new, GB_OBJECT parent; GB_BOOLEAN hidden)
  ...
  else if (GB.Is(parent, CLASS_Window))
  {
    ...
    if (!menuBar)
    {
      if (topLevel->isA("QMainWindow"))
        menuBar = ((Q3MainWindow *)topLevel)->menuBar();
      else
  ...
  if (GB.Is(parent, CLASS_Menu))
  {
    ...
    item->parentMenu = NULL; /* plus tard */
    item->parentMenuBar = NULL;
    item->parent = pmenu;
...
BEGIN_PROPERTY(CMENUITEM_enabled)
  ...
  {
    if (READ_PROPERTY)
      GB.ReturnBoolean(item->parentMenuBar->isItemEnabled(item->id));
    else
    {
      enabled = VPROP(GB_BOOLEAN);
      item->parentMenuBar->setItemEnabled(item->id, enabled);
  ...
  {
    if (READ_PROPERTY)
      GB.ReturnBoolean(item->parentMenu->isItemEnabled(item->id));
    else
    {
      enabled = VPROP(GB_BOOLEAN);
      item->parentMenu->setItemEnabled(item->id, enabled);
...
BEGIN_PROPERTY(CMENU_checked)
  ...
  {
    if (READ_PROPERTY)
      GB.ReturnBoolean(item->parentMenu->isItemChecked(item->id));
    else
    {
	    bool change = CMENU_is_visible(THIS);
...
BEGIN_METHOD(CMENU_popup, GB_INTEGER x; GB_INTEGER y)
  ...
  if (CMENU_is_popup(item))
  {
    ...
    if (popup)
    {
      ...
      qApp->processEvents(QEventLoop::ExcludeUserInput, 0);
		}
  }
