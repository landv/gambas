...
[
  AM_INIT_AUTOMAKE($1, 1.9.44)
  AM_CONFIG_HEADER(config.h)
  AC_DEFINE(GAMBAS_FULL_VERSION, 0x01090044, [Full Gambas version])
  AC_DEFINE(GAMBAS_PCODE_VERSION, 0x01090034, [Gambas bytecode version])
])
...
[
  ...
  if test "$gb_cv_path_qt_moc" = no; then
    AC_MSG_ERROR([QT moc compiler not found. Try --with-moc option.])
  fi
...
[
  ...
  AC_CHECK_LIB(X11, XOpenDisplay, echo -n, [
    AC_MSG_WARN([libX11 not found. Check 'config.log' for more details.])
    touch DISABLED
    ], $X_LIBS)

  AC_CHECK_LIB(Xext, XShmAttach, echo -n, [
    AC_MSG_WARN([libXext not found. Check 'config.log' for more details.])
    touch DISABLED
    ], $X_LIBS)
])
