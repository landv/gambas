...
static int do_query(SqliteDatabase * conn, const char *error, Dataset ** pres,
...
static int exec_query(DB_DATABASE * db, char *query, DB_RESULT * result,
											char *err)
{
	return do_query((SqliteDatabase *) db->handle, err, (Dataset **) result,
									query, 0);
}
...
static int query_fill(DB_RESULT result, int pos, GB_VARIANT_VALUE * buffer,
											int next)
{
	Dataset *res = (Dataset *) result;
	int i;
	...
	for (i = 0; i < res->fieldCount(); i++)
	{
		...
		//fprintf(stderr, "query_fill: %d: %d -> %ld: %s\n", i, res->fieldType(i), value.value.type, data);
...
{
	return do_query((SqliteDatabase *) db->handle,
									"Unable to begin transaction: &1", NULL, "BEGIN", 0);
}
...
{
	return do_query((SqliteDatabase *) db->handle,
									"Unable to commit transaction: &1", NULL, "COMMIT", 0);
}
...
{
	return do_query((SqliteDatabase *) db->handle,
									"Unable to rollback transaction: &1", NULL, "ROLLBACK", 0);
}
...
static int table_init(DB_DATABASE * db, char *table, DB_INFO * info)
{
	...
	if (do_query
			(conn, "Unable to get table fields: &1", &res, qfield, 1, table))
		return TRUE;
...
static int table_index(DB_DATABASE * db, char *table, DB_INFO * info)
{
	...
	if (do_query
			(conn, "Unable to get primary index: &1", &res, qindex1, 1, table))
		return TRUE;
	...
	if (n <= 0)
	{
		...
		if (do_query
				(conn, "Unable to get primary index: &1", &res,
		...
		info->nindex = 1;
		GB.Alloc((void **) &info->index, sizeof(int));
	...
	for (int i = 0; i < n; i++)
	{
		if (strstr(r->records[i][1].get_asString().data(), "autoindex") != NULL)
		{
			...
			if (do_query
					(conn, "Unable to get information on primary index: &1", &res,
					 qindex2, 1, sql))
			{
				res->close();
				GB.FreeString(&sql);
			...
			GB.Alloc((void **) &info->index, sizeof(int) * info->nindex);
...
static int table_primary_key(DB_DATABASE * db, char *table, char ***primary)
{
	...
	if (do_query
			(conn, "Unable to get primary key: &1", &res, qindex1, 1, table))
		return TRUE;
	...
	for (i = 0; i < n; i++)
	{
		if (strstr(r->records[i][1].get_asString().data(), "autoindex"))
		{
			...
			if (do_query
					(conn, "Unable to get primary key: &1", &res, qindex2, 1, sql))
			{
				res->close();
				GB.FreeString(&sql);
				return TRUE;
	...
	{
		if (do_query
				(conn, "Unable to get primary key: &1", &res,
				 "PRAGMA table_info('&1')", 1, table))
			return TRUE;
...
{
	return
		do_query((SqliteDatabase *) db->handle, "Unable to delete table: &1",
...
static int field_exist(DB_DATABASE * db, char *table, char *field)
{
	...
	if (do_query
			(conn, "Unable to find field: &1.&2", &res, query, 2, table, field))
	{
		return FALSE;
	}
											...
											DB_INDEX * info)
{
	...
	if (do_query
			(conn, "Unable to get index info for table: &1", &res, qindex1, 1,
			 table))
		return TRUE;
	...
	if (do_query
			(conn, "Unable to get index info for : &1", &res, qindex2, 1, index))
	{
		res->close();
		return TRUE;
	}
...
{
	return
		do_query((SqliteDatabase *) db->handle, "Unable to delete index: &1",
												...
												DB_INDEX * info)
{
	...
	return do_query((SqliteDatabase *) db->handle, "Cannot create index: &1",
									NULL, DB.Query.Get(), 0);
}
...
_CREATE_DATABASE:
	...
	if (!do_query
			(&conn2, "Unable to initialise database", NULL,
			 "CREATE TABLE GAMBAS (FIELD1 TEXT)", 0))
		do_query(&conn2, NULL, NULL, "DROP TABLE GAMBAS", 0);
