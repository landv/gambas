...
static int do_query(PGconn *conn, const char *error, PGresult **pres,
...
static long db_version(PGconn *conn)
{
  //Check db version
  const char *vquery =
...
static int open_database(DB_DESC *desc, DB_DATABASE *db)
{
  ...
  if (!do_query(conn, NULL, &res, query, 0))
  {
...
{
  return do_query((PGconn *)db->handle, err, (PGresult **)result, query, 0);
}
...
static int field_index(DB_RESULT Result, char *name, DB_DATABASE *db)
{
  ...
  int numfields, oid;
  char *qfield =
    "select oid from pg_class where relname = lower('&1') "
  ...
  if (fld){
     ...
     if (db->version > 70399){ // version 7.4.?
        ...
        if (do_query(conn, "Unable to get OID for table &1", &oidres, qfield, 1, table)){
	    GB.FreeString(&table);
	    return -1;
        }
...
{
  return do_query((PGconn *)db->handle, "Unable to begin transaction: &1", NULL, "BEGIN", 0);
}
...
{
  return do_query((PGconn *)db->handle, "Unable to commit transaction: &1", NULL, "COMMIT", 0);
}
...
{
  return do_query((PGconn *)db->handle, "Unable to rollback transaction: &1", NULL, "ROLLBACK", 0);
}
...
static int table_init(DB_DATABASE *db, char *table, DB_INFO *info)
{
  char *qfield =
    "select pg_attribute.attname, pg_attribute.atttypid::int,pg_attribute.atttypmod "
      ...
      "where pg_class.relname = lower('&1') "
  ...
  if (do_query(conn, "Unable to get table fields: &1", &res, qfield, 1, table))
    return TRUE;
...
static int table_index(DB_DATABASE *db, char *table, DB_INFO *info)
{
  const char *qindex =
    "select pg_att1.attname, pg_att1.atttypid::int, pg_cl.relname "
      "from pg_attribute pg_att1, pg_attribute pg_att2, pg_class pg_cl, pg_index pg_ind "
        "where pg_cl.relname = lower('&1_pkey') AND pg_cl.oid = pg_ind.indexrelid "
  ...
  for (i = 0; i < n; i++)
  {
    ...
    {
      if (strcasecmp(info->field[j].name, PQgetvalue(res, i, 0)) == 0)
      {
        info->index[i] = j;
        /*printf("index[%d] = %d\n", i, j);*/
        break;
...
static int table_exist(DB_DATABASE *db, char *table)
{
  ...
  //if (1) //version < 70400)
  //{
    if (do_query(conn, "Unable to check table: &1", &res, query73, 1, table))
      return FALSE;
  /*}
  else
  {
...
static long table_list_73(DB_DATABASE *db, char ***tables)
{
  const char *query =
    "select relname from pg_class where (relkind = 'r' or relkind = 'v') "
    "and (relnamespace not in (select oid from pg_namespace where nspname = 'information_schema'))";
  ...
  if (do_query(conn, "Unable to get tables: &1", &res, query, 0))
    return -1;
...
#if 0
static long table_list_74(DB_DATABASE handle, char ***tables)
{
  const char *query1 =
...
static int table_is_system(DB_DATABASE *db, char *table)
{
  ...
  const char *query2 =
    "select 1 from pg_views where viewname = lower('&1') and schemaname = 'public'";
  ...
  if (do_query(conn, "Unable to check table: &1", &res, query2, 1, table))
    return FALSE;
...
{
  return
    do_query((PGconn *)db->handle, "Unable to delete table: &1", NULL,
...
static int field_exist(DB_DATABASE *db, char *table, char *field)
{
  ...
  if (do_query((PGconn *)db->handle, "Unable to check field: &1", &res, query, 2, table, field))
    return FALSE;
...
static long field_list(DB_DATABASE *db, char *table, char ***fields)
{
  ...
  if (do_query((PGconn *)db->handle, "Unable to get fields: &1", &res, query, 1, table))
    return -1;
...
static int field_info(DB_DATABASE *db, char *table, char *field, DB_FIELD *info)
{
  ...
  if (do_query((PGconn *)db->handle, "Unable to get field info: &1", &res, query, 2, table, field))
    return TRUE;
    	...
    	else
    	{
				switch(info->type)
				{
					...
					default:
					  ...
					  DB.Query.Init();
						if (!unquote(val, PQgetlength(res, 0, 4), DB.Query.AddLength))
							val = DB.Query.Get();
...
static int index_exist(DB_DATABASE *db, char *table, char *index)
{
  const char *query =
    ...
    "where pg_class2.relname = lower('&1') "
  ...
  if (do_query((PGconn *)db->handle, "Unable to check index: &1", &res, query, 2, table, index))
    return FALSE;
...
static long index_list(DB_DATABASE *db, char *table, char ***indexes)
{
  ...
  if (do_query((PGconn *)db->handle, "Unable to get indexes: &1", &res, query, 1, table))
    return TRUE;
...
static int index_info(DB_DATABASE *db, char *table, char *index, DB_INDEX *info)
{
  ...
  if (do_query((PGconn *)db->handle, "Unable to get index info: &1", &res, query, 2, table, index))
    return TRUE;
  ...
  if (do_query((PGconn *)db->handle, "Unable to get index info: &1", &res, query_field, 1, indexrelid))
    return TRUE;
...
{
  return
    do_query((PGconn *)db->handle, "Unable to delete index: &1", NULL,
...
static int index_create(DB_DATABASE *db, char *table, char *index, DB_INDEX *info)
{
  ...
  return do_query((PGconn *)db->handle, "Cannot create index: &1", NULL, DB.Query.Get(), 0);
}
...
static int database_exist(DB_DATABASE *db, char *name)
{
  ...
  if (do_query(conn, "Unable to check database: &1", &res, query, 1, name))
    return FALSE;
...
static long database_list(DB_DATABASE *db, char ***databases)
{
  ...
  if (do_query(conn, "Unable to get databases: &1", &res, query, 0))
    return -1;
...
{
  return
    do_query((PGconn *)db->handle, "Unable to delete database: &1", NULL,
...
{
  return
    do_query((PGconn *)db->handle, "Unable to create database: &1", NULL,
...
static int user_exist(DB_DATABASE *db, char *name)
{
  ...
  if (do_query((PGconn *)db->handle, "Unable to check user: &1", &res, query, 1, name))
    return FALSE;
...
static long user_list(DB_DATABASE *db, char ***users)
{
  ...
  if (do_query((PGconn *)db->handle, "Unable to get users: &1", &res, query, 0))
    return -1;
...
static int user_info(DB_DATABASE *db, char *name, DB_USER *info)
{
  ...
  if (do_query((PGconn *)db->handle, "Unable to get user info: &1", &res, query, 1, name))
    return TRUE;
  ...
  if (!do_query((PGconn *)db->handle, NULL, &res, query_passwd, 1, name))
  {
...
{
  return
    do_query((PGconn *)db->handle, "Unable to delete user: &1", NULL,
      "drop user \"&1\"", 1, name);
}
...
static int user_create(DB_DATABASE *db, char *name, DB_USER *info)
{
  ...
  /*_print_query = TRUE;*/
  return do_query((PGconn *)db->handle, "Cannot create user: &1", NULL, DB.Query.Get(), 0);
}
...
static int user_set_password(DB_DATABASE *db, char *name, char *password)
{
  ...
  return
    do_query((PGconn *)db->handle, "Cannot change user password: &1",
      NULL, DB.Query.Get(), 0);
}
