...
PUBLIC void SUBR_instr(void)
{
  ...
  pos = STRING_search(ps, ls, pp, lp, is, right);
...
static void get_subst(int np, char **str, long *len)
{
...
PUBLIC void SUBR_replace(void)
{
  ...
  if (ls)
  {
    ...
    if (lp)
    {
      for(;;)
      {
        pos = STRING_search(ps, ls, pp, lp, 1, FALSE);
        if (pos == 0)
          break;
...
PUBLIC void SUBR_split(void)
{
  ...
  if (*str)
  {
    ...
    CARRAY_split(array, str, sep, esc, no_void);
