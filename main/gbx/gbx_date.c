...
PUBLIC void DATE_add(VALUE *date, int period, long val)
{
  ...
  switch(period)
  {
    case DP_SECOND:
      add_date = val / 86400;
      add_time = (val - (add_date * 86400)) * 1000;
      goto __ADD_DATE_TIME;

    case DP_MINUTE:
      add_date = val / 1440;
      add_time = (val - (add_date * 1440)) * 60000;
      goto __ADD_DATE_TIME;

    case DP_HOUR:
      add_date = val / 24;
      add_time = (val - (add_date * 24)) * 3600000;
      goto __ADD_DATE_TIME;
...
__ADD_DATE_TIME:
  ...
  date->_date.date = (long)new_date;
  date->_date.time = (long)new_time;
  return;
