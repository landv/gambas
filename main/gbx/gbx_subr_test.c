...
PUBLIC void SUBR_comp(void)
{
  ...
  int result= 0; // prevents a gcc warning
  TYPE type, typem;
  VALUE *P1, *P2;
  boolean variant;
  short op;
...
PUBLIC void SUBR_strcomp(void)
{
  ...
  if (NPARAM == 3)
  {
    comp = SUBR_get_integer(&PARAM[2]);
    if (comp != GB_COMP_TEXT && comp != GB_COMP_BINARY)
      THROW(E_ARG);
  }

  RETURN->type = T_INTEGER;

  if (comp == GB_COMP_BINARY)
    RETURN->_integer.value = STRING_comp_value(&PARAM[0], &PARAM[1]);
  else
    RETURN->_integer.value = STRING_comp_value_ignore_case(&PARAM[0], &PARAM[1]);
