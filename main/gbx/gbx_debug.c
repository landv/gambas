...
int DEBUG_can_be_used_like_an_array(void *object)
{
	CLASS *class;
	CLASS_DESC *desc;
  char type;
...
PUBLIC char *DEBUG_get_backtrace(void)
{
  ...
  if (CP)
    sprintf(COMMON_buffer, "%s\n", DEBUG_get_current_position());
  else
    sprintf(COMMON_buffer, "?\n");
    
	STRING_add(&result, COMMON_buffer, 0);
  ...
  for (i = 0; i < (STACK_frame_count - 1); i++)
  {
    ...
    if (context->pc)
    {
      ...
      if (calc_line_from_position(context->cp, context->fp, context->pc, &line))
        ...
        sprintf(COMMON_buffer, "%s.%s.%d\n", context->cp->name, context->fp->debug->name, line);
    }
    else if (context->cp)
      sprintf(COMMON_buffer, "?\n");
