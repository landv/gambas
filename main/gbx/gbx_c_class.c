...
BEGIN_METHOD(object_is, GB_OBJECT object; GB_STRING klass)

  void *object = VARG(object);
  CLASS *klass = CLASS_look(GB_ToZeroString(ARG(klass)));
  ...
  GB_ReturnBoolean(OBJECT_class(object) == klass || CLASS_inherits(OBJECT_class(object), klass));
