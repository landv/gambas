...
static void *collection_get_key(CCOLLECTION *col, const char *key, long len)
{
  if (!key || !*key)
  {
    GB_Error("Void key");
...
static void *collection_add_key(CCOLLECTION *col, const char *key, long len)
{
  if (!key || !*key)
  {
    GB_Error("Void key");
...
static void collection_remove_key(CCOLLECTION *col, const char *key, long len)
{
  void *value;
  HASH_NODE *last;
