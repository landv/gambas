...
PUBLIC CLASS *CLASS_look(const char *name, int len)
{
  ...
  {
    if (TABLE_find_symbol(arch->classes, name, len, (SYMBOL **)&csym, NULL))
    {
      #if DEBUG_COMP
  ...
  if (TABLE_find_symbol(&_global_table, name, len, (SYMBOL **)&csym, &index))
  {
    #if DEBUG_COMP
...
PUBLIC CLASS *CLASS_find(const char *name)
{
  ...
  if (!_global && !ARCHIVE_get_current(&arch))
  {
    TABLE_add_symbol(arch->classes, name, len, (SYMBOL **)&csym, NULL);
  ...
  {
    TABLE_add_symbol(&_global_table, name, len, (SYMBOL **)&csym, &index);
    #if DEBUG_LOAD || DEBUG_COMP
...
static int sort_desc(ushort *i1, ushort *i2)
{
  ...
  return TABLE_compare_ignore_case(
    ...
    for (j = 0; j < n_desc; j++)
    {
      if (CLASS_is_native(class))
        ...
        name = desc[j].gambas.name;
			...
			{
				#if DEBUG_DESC
				fprintf(stderr, "%s: %p -> %p\n", name, class->table[ind].desc, &desc[j]);
				#endif
