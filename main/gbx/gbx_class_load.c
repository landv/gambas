...
static char *get_section(char *sec_name, char **section, long one, short *psize, char *swapcode)
{
...
static void check_version(long loaded)
{
  long current = GAMBAS_PCODE_VERSION;

  if ((GAMBAS_FULL_VERSION & 0xFFFF0000) == 0x109)
  {
    current &= ~0xFFFF;
    loaded &= ~0xFFFF;
  }
  ...
  if (loaded > current)
    THROW(E_CLASS, ClassName, "Interpreter too old. Please upgrade.", "");
  if (loaded < current)
...
PUBLIC void CLASS_load_without_init(CLASS *class)
{
  ...
  if (header->magic != OUTPUT_MAGIC)
  {
  	...
  	fprintf(stderr, "Dumping class file at /tmp/gambas-bad-header.out: %ld bytes\n", len_data);
  ...
  class->load->unknown = (char **)get_section("unknown section", &section, sizeof(long), &n_unknown, "l");
  ...
  //fprintf(stderr, "%s: info->flag = %d class->auto_create = %d\n", class->name, info->flag, class->auto_create);
  ...
  {
  	offset = (long)class->load->class_ref[i];
  	if (offset >= 0)
  ...
  /* Instanciation callbacks - If the class is not static */
