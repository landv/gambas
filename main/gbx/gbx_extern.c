...
static lt_dlhandle get_library(char *name)
{
  ...
  TABLE_add_symbol(_table, name, strlen(name), (SYMBOL **)&esym, NULL);
  if (!esym->handle)
  {
    /* !!! Must add the suffix !!! */
...
PUBLIC void EXTERN_call(void)
{
  ...
  if (nparam)
  {
    ...
    sign = (TYPE *)ext->param;
    parg = (long *)&args;
    marg = parg + sizeof(ARGS) / sizeof(long);
  ...
  switch (ext->type)
  {
    ...
    case T_SHORT:
    case T_INTEGER:
      GB_ReturnInteger(CAST(long, func)(args));
      break;
