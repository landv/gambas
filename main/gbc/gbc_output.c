...
static bool _swap;
/*
#define START_SECTION()  long pos = write_long(0);
#define END_SECTION()    write_pad(); write_long_at(pos, get_pos() - pos - sizeof(long));
*/
...
static long get_string(const char *string, int len)
{
  ...
  if (!TABLE_add_symbol(StringTable, string, len, (SYMBOL **)&sym, NULL))
  {
...
/*static void swap_value(char *p, int n)
{
  int i, j, max;
  char c;
...
static void write_byte(unsigned char val)
{
  ...
  /* MSB & LSB */
  if (fwrite(&val, sizeof(char), 1, File) != 1)
    THROW("Write error");
}


static void write_short(unsigned short val)
{
  #ifdef DEBUG_MORE
  printf("%ld : i %u 0x%X\n", get_pos(), val, val);
  #endif
...
static long write_long(unsigned long val)
{
  long pos = get_pos();
...
static long write_long_long(unsigned long long val)
{
  long pos = get_pos();
...
static void write_long_at(long pos, unsigned long val)
{
  long prev = get_pos();
...
static void end_section(void)
{
  ...
  if (PosStartSection)
  {
    ...
    write_long_at(PosStartSection, len);
...
static void output_method(void)
{
  ...
  for (i = 0; i < n; i++)
  {
    ...
    /* addr_code */
    write_long(ARRAY_count(func->code));
    /* desc_param */
    write_long(0);
    /* desc_local */
    write_long(0);
  ...
  for (i = 0; i < ARRAY_count(Class->function); i++)
  {
    ...
    if (func->name != NO_SYMBOL)
    {
      ...
      for (j = 0; j < func->nlocal + func->nparam; j++)
      {
        ...
        TABLE_add_symbol(table, csym->symbol.name, csym->symbol.len, (SYMBOL **)&osym, NULL);
