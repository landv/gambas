...
PUBLIC void ARCH_exit(void)
{
  ...
  /* �riture de la table des symboles */
...
PUBLIC void ARCH_add_file(const char *path)
{
  char *rel_path;
  ARCH_SYMBOL *sym;
  FILE *file;
  ...
  TABLE_add_symbol(arch_table, rel_path, strlen(rel_path), (SYMBOL **)&sym, NULL);
  sym->pos = get_pos();
