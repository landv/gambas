...
typedef
	struct {
		...
		int name;             // index of name in class symbol table

		char nparam;          // Arguments count
		char npmin;           // Minimum arguments count
		char vararg;          // If the function takes a variable number of arguments
		char _reserved;
		
		PARAM *local;         // Local variables list
		PARAM *param;         // Argument list

		PATTERN *start;       // Start compile function from there
		
		int line;             // ...which is that line number
		
		short nlocal;         // Local variables count
		short nctrl;          // Number of local variables needed by control structures
		
		ushort *code;         // Generated bytecode
		
		ushort ncode;         // Number of bytecode instructions
		short _reserved2;

		short *pos_line;      // Position of each code line

		short last_code;      // Last compiled bytecode position
		short stack;          // Amount of stack needed by the function
		
		short finally;        // Where FINALLY is located
		short catch;          // Where CATCH is located
		}
	FUNCTION;
