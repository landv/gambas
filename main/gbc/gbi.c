...
static void init(void)
{
  ...
  if (lt_dlinit())
    error2("Cannot initialize plug-in management", lt_dlerror(), TRUE);
...
static void analyze(const char *comp)
{
  ...
  if (!out_info)
    error2("Cannot write file", path_info, TRUE);

  //puts(path_list);
  out_list = fopen(path_list, "w");
  if (!out_list)
    error2("Cannot write file", path_list, TRUE);
...
int main(int argc, char **argv)
{
      ...
      )
  {
    ...
    if (dir == NULL)
      error2("Cannot read directory", _lib_path, TRUE);
