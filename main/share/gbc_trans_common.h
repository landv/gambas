...
typedef
  struct {
    long index;               /* index dans la table des symboles */
    TYPE type;                /* type de donn�s */
    long value;               /* valeur associ� */
    TRANS_ARRAY array;        /* dimensions du tableau si d�laration */
    PATTERN *init;            /* initialization code */
    long long lvalue;         /* The value of a LONG constant */
    unsigned is_new : 1;      /* s'il faut instancier un objet */
    unsigned is_integer : 1;  /* si la valeur de la constante est enti�e */
    }
  PACKED
  TRANS_DECL;
