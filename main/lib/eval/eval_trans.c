...
PUBLIC boolean TRANS_get_number(long index, TRANS_NUMBER *result)
{
  ...
  {
    result->type = T_INTEGER;
    result->ival = ((GB_INTEGER *)&value)->value;
  }
  else if (value.type == T_LONG)
  {
    result->type = T_LONG;
    result->lval = ((GB_LONG *)&value)->value;
  }
  else
  {
    result->type = T_FLOAT;
    result->dval = ((GB_FLOAT *)&value)->value;
  }
...
PUBLIC boolean TRANS_type(int mode, TRANS_DECL *result)
{
  ...
  /* Ne pas remplir la structure de zéros */

  /* Attention ! Problème du tableau d'objet ! */
