  ...
  */
  ...
  if (op == RS_EXCL)
  {
    /*if (!PATTERN_is_identifier(get_last_pattern(1)))
      THROW("Operator '!' must be followed by an identifier.");*/

    sym = TABLE_get_symbol(EVAL->table, PATTERN_index(get_last_pattern(1)));
    TABLE_add_symbol(EVAL->string, sym->name, sym->len, NULL, &index);
...
static void analyze_single(int op)
{
  ...
  {
    long index = ARRAY_count(EVAL->tree);
    PATTERN last;
  ...
  */
  ...
  else
  {
    ...
    if (nparam_post < info->min_param)
      THROW("Not enough arguments"); /* , info->name);*/
    else if (nparam_post > info->max_param)
      THROW("Too many arguments"); /*, info->name);*/
