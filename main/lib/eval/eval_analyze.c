/***************************************************************************
  ...
  (c) 2000-2005 Benoit Minisini <gambas@users.sourceforge.net>
...
static void get_symbol(PATTERN pattern, const char **symbol, long *len)
{
  SYMBOL *sym;
  int type = PATTERN_type(pattern);
  long index = PATTERN_index(pattern);
...
static long get_utf8_length(const char *s, long l)
{
  long len;
  long i;
...
{
  *((PATTERN *)ARRAY_add(&EVAL->pattern)) = PATTERN_make(type, index);
}
...
static void add_end_pattern(void)
{
  ...
  TABLE_add_symbol(EVAL->string, &EVAL->source[READ_source_ptr], EVAL->len - READ_source_ptr, NULL, &index);
...
PUBLIC void EVAL_analyze(const char *src, long len, EVAL_ANALYZE *result)
{
	int nspace = 0;
  ...
  if (len > 0)
  {
		...
		EVAL->source = (char *)src;
		EVAL->len = len;
		EVAL->analyze = TRUE;

		ARRAY_create(&EVAL->pattern);

		TABLE_create(&EVAL->table, sizeof(EVAL_SYMBOL), TF_NORMAL);
