...
#if 0
static void EVAL_enter(VALUE_FUNCTION *value, int nparam)
{
...
static void EVAL_exec(VALUE_FUNCTION *func, int nparam)
{
  STACK_push_frame(&EXEC_current);
...
{
  ALLOC(&expr->pattern, sizeof(PATTERN) * (16 + (expr->len >> 1)), "EVAL_start");
  expr->pattern_count = 0;

  TABLE_create(&expr->table, sizeof(EVAL_SYMBOL), TF_IGNORE_CASE);
