...
//   DRAW->Draw.ZoomedImage(THIS, image, zoom, 
// 		VARGOPT(x, 0), VARGOPT(y, 0),
// 		VARGOPT(sx, 0), VARGOPT(sy, 0), VARGOPT(sw, -1), VARGOPT(sh, -1));
  ...
  border = DRAW->Line.GetStyle(THIS) != 0;
...
GB_DESC CDrawDesc[] =
{
  ...
  GB_STATIC_METHOD("RichTextWidth", NULL, CDRAW_rich_text_width, "(Text)s"),
  GB_STATIC_METHOD("RichTextHeight", NULL, CDRAW_rich_text_height, "(Text)s[(Width)i]"),
