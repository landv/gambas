...
typedef
	struct {
		...
		void (*PrintError)(FILE *where, bool msgonly);
		void (*SaveError)(void *);
		void (*RestoreError)(void *);
		void (*ToString)(GB_VALUE *value, char **addr, long *len);
		int (*FormatDate)(GB_DATE_SERIAL *date, int fmt_type, const char *fmt, long len_fmt, char **str, long *len_str);
		int (*FormatNumber)(double number, int fmt_type, const char *fmt, long len_fmt, char **str, long *len_str, bool local);
		bool (*GetValue)(const char *sym, long len, GB_VARIANT *ret);
		void (*GetArrayValue)(GB_ARRAY array, int index, GB_VALUE *value);
		int (*EnumCollection)(GB_COLLECTION col, GB_VARIANT *value, char **key, long *len);
