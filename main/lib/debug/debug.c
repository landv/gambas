...
#define WARNING(_msg) fprintf(stderr, "Debugger: " _msg)
...
static bool init_breakpoint(DEBUG_BREAK *brk)
{
  ...
  //fprintf(stderr, "init_breakpoint: id = %d\n", brk->id);
	...
	if (brk->class->state == CS_NULL)
		return TRUE;

  if (CLASS_is_native(brk->class) || !brk->class->debug)
  {
    WARNING("init_breakpoint: No debugging information\n");
    return TRUE;
  }

  if (calc_position_from_line(brk->class, brk->line, &func, &addr))
  {
  	WARNING("init_breakpoint: cannot calc position\n");
  ...
  {
  	WARNING("init_breakpoint: Not a line beginning\n");
  ...
  if (*addr & 0xFF)
  {
  	WARNING("init_breakpoint: Breakpoint already set\n");
...
static boolean set_breakpoint(CLASS *class, ushort line)
{
  ...
  init_breakpoint(brk);
  	//fprintf(stderr, "Breakpoint is pending.\n");
...
static void command_set_breakpoint(const char *cmd)
{
  ...
  if (sscanf(cmd, "+%64[^.].%hu", class_name, &line) != 2)
    WARNING("command_set_breakpoint: syntax error\n");
  else
  {
...
static void command_unset_breakpoint(const char *cmd)
{
  ...
  if (sscanf(cmd, "-%64[^.].%hu", class_name, &line) != 2)
    WARNING("command_unset_breakpoint: Syntax error\n");
  else
  {
...
PUBLIC void DEBUG_backtrace(FILE *out)
{
  ...
  {
    fprintf(out, "*");
    GB_DEBUG.PrintError(_out, FALSE);
    //ERROR_print_at(_out);
  }
  else
    fprintf(out, "\n");
...
static void command_local(const char *cmd)
{
  int i;
  LOCAL_SYMBOL *lp;
  bool nl = TRUE;
...
static void command_object(const char *cmd)
{
  int i;
  GLOBAL_SYMBOL *gp;
...
/*
static void command_info(char *cmd)
{
  printf("w:\n");
  command_where(cmd);
  printf("l:\n");
...
static void command_eval(const char *cmd)
{
  ...
  if (!val)
	...
	switch(*cmd)
	{
		case '?':
	    fprintf(_out, "=");
		  PRINT_value(_out, val, TRUE);
		  break;
		  
		case '!':
		  PRINT_value(_out, val, FALSE);
		  break;
		  
		case '#':
		  PRINT_object(_out, val);
		  break;
	}
