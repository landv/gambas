...
static bool end(FUNCTION *func, int index)
{
	STR_free_later(NULL);
	JIT_print("__RETURN:\n");
		...
		if (index != NO_SYMBOL)
		{
			...
			switch (CLASS_DESC_get_type(desc))
			{
				...
				case CD_VARIABLE:
					...
					expr = STR_copy(peek(-1, (TYPE)class, NULL, NULL));
					pop_stack(1);
	...
	if (!op)
	{
		switch(code & 0xFF00)
		{
			...
			case C_GT: case C_LT: case C_GE: case C_LE:
				push_subr(code, "CALL_SUBR_UNKNOWN");
				break;
		}
...
static void push_subr_conv(ushort code)
{
	...
	expr = STR_copy(peek(-1, conv, NULL, NULL));
	pop_stack(1);
	push(conv, "(%s)", expr);
	STR_free(expr);
}
...
static void push_subr_len(ushort code)
{
	...
	expr = STR_copy(peek(-1, T_STRING, NULL, NULL));
	pop_stack(1);
...
/*static void push_subr_left(ushort code)
{
	...
	if (_stack_current >= 2)
	{
		expr_len = peek(-1, T_INTEGER, NULL, NULL);
		pop_stack(1);
	}
	
	type = get_type(-1);
	expr_str = peek(-1, T_CSTRING, NULL, NULL);
	pop_stack(1);
...
static void push_subr_isnan(ushort code)
{
	...
	expr = STR_print("%s(%s) != 0", func, peek(-1, T_FLOAT, NULL, NULL));
...
static void push_subr_math(ushort code)
{
	...
	expr = STR_copy(peek(-1, T_FLOAT, NULL, NULL));
	pop_stack(1);
