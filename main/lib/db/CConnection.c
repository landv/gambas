...
static void make_query_get_param(int index, char **str, long *len)
{
	if (index == 1)
...
static char *make_query(CCONNECTION *_object, char *pattern, long len, int narg, GB_VALUE *arg)
{
	char *query;
	char *keyword;
	char buffer[32];
...
static char *get_query(char *prefix, CCONNECTION *_object, char *table, long len_table, char *query, long len_query, GB_VALUE *arg)
{
  q_init();
