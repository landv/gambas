...
static void check_blob(CRESULT *_object, int field)
{
  GB_VARIANT val;
	...
	//fprintf(stderr, "check_blob: %d: %d\n", field, THIS->buffer[field].type);
...
BEGIN_METHOD(CRESULT_put, GB_VARIANT value; GB_STRING field)
  ...
  if (index < 0)
	...
	if (type == DB_T_BLOB)
	{
		...
		{
			GB_STRING *str = (GB_STRING *)ARG(value);

	    if (GB.Conv((GB_VALUE *)ARG(value), GB_T_STRING))
  	    return;
...
static CBLOB *make_blob(CRESULT *result, int field)
{
  ...
  GB.New((void **)&_object, CLASS_Blob, NULL, NULL);
  ...
  //GB.Ref(result);
	...
	//GB.UnrefKeep((void **)&_object, FALSE);
	return BLOB;
}

static void set_blob(CBLOB *_object, char *data, long length)
{
	//fprintf(stderr, "set_blob: '%s'\n", data);
