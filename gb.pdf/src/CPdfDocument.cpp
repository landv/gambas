...
#if POPPLER_VERSION_0_5
	GB.ReturnInteger((int)THIS->page->getMediaWidth());
#else
	GB.ReturnInteger((int)THIS->page->getWidth());
#endif
...
#if POPPLER_VERSION_0_5
	GB.ReturnInteger((int)THIS->page->getMediaHeight());
#else
	GB.ReturnInteger((int)THIS->page->getHeight());
#endif
...
#if POPPLER_VERSION_0_5
	if (w < 0)
		w = (int)THIS->page->getMediaWidth();

	if (h < 0)
		h = (int)THIS->page->getMediaHeight();

	if (x<0) x=0;
	if (y<0) y=0;
	if (w<1) w=1;
	if (h<1) h=1;
	if ( (x+w) > (int)THIS->page->getMediaWidth() ) w=(int)THIS->page->getMediaWidth()-x;
	if ( (y+h) > (int)THIS->page->getMediaHeight() ) h=(int)THIS->page->getMediaHeight()-y;
#else
	if (w < 0)
		w = (int)THIS->page->getWidth();

	if (h < 0)
		h = (int)THIS->page->getHeight();

	if (x<0) x=0;
	if (y<0) y=0;
	if (w<1) w=1;
	if (h<1) h=1;
	if ( (x+w) > (int)THIS->page->getWidth() ) w=(int)THIS->page->getWidth()-x;
	if ( (y+h) > (int)THIS->page->getHeight() ) h=(int)THIS->page->getHeight()-y;
#endif
