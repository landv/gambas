...
BEGIN_METHOD(GLLINESTIPPLE, GB_INTEGER factor; GB_INTEGER pattern)
	...
	glLineStipple(VARG(factor), (GLushort ) VARG(pattern));
...
/*	GLubyte mask[32*4];
	...
	for (i=0;i<32;i++)
		mask[0+(i*4)] = *((GLint *)GB.Array.Get(iArray,i));

	glPolygonStipple(mask);
*/
	WARNING("glPolygonStipple() Not implemented !");
...
BEGIN_METHOD(GLRASTERPOS4F, GB_FLOAT x; GB_FLOAT y; GB_FLOAT z; GB_FLOAT w)
...
BEGIN_METHOD(GLRASTERPOS4I, GB_INTEGER x; GB_INTEGER y; GB_INTEGER z; GB_INTEGER w)
