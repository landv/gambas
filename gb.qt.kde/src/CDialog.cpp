...
static QString get_filter(void)
{
  ...
  if (dialog_filter)
  {
    /*for (i = 0; i < GB.Array.Count(dialog_filter); i++)
    {
      if (i)
        s += "\n";
      f = TO_QSTRING(*((char **)GB.Array.Get(dialog_filter, i)));
      f = f.stripWhiteSpace();
      p = f.findRev(" (");
      if (p >= 0)
      {
        extp = ext = f.mid(p + 2, f.length() - p - 3);
        extp = extp.replace(";", " ");
        f = extp + "|" + f.left(p) + " (" + ext + ")";
      }

      //qDebug("filter = %s", f.latin1());
...
BEGIN_PROPERTY(CDIALOG_font)
  ...
  if (READ_PROPERTY)
    GB.ReturnObject(QT.CreateFont(dialog_font));
  else
  {
