...
void FUNCTION_NAME(void *_object) //(QFrame *cont)
{
  ...
  //	qDebug("CCONTAINER_arrange: %p %s %d %d %d", THIS, GB.GetClassName(THIS), IS_WIDGET_VISIBLE(cont), arr->locked, arr->mode);
		...
		for(i = 0; i < 3; i++)
		{
			...
			if (autoresize)
			{
				switch(arr->mode)
				{
					case ARRANGE_HORIZONTAL:
						#ifndef QNAMESPACE_H
						fprintf(stderr, "CCONTAINER_arrange: %s: x = %d autoresize (%d, %d) pad %d spc %d [%d] rtl %d\n", 
