...
BEGIN_METHOD(CICONVIEW_new, GB_OBJECT parent)
  ...
  wid->setArrangementMode(QIconView::LeftToRight);

  wid->setWordWrapIconText(false);
  wid->setGridX(128);
  wid->setGridY(64);
  wid->setSpacing(8);
  wid->show();
  ...
  if (READ_PROPERTY)
    GB.ReturnInteger(
    	CCONST_convert(WIDGET->selectionMode(), 3, 
    		QIconView::NoSelection, SELECT_NONE, 
    		QIconView::Single, SELECT_SINGLE, 
    		QIconView::Multi, SELECT_MULTIPLE, 
    		SELECT_NONE));
  else
    WIDGET->setSelectionMode((QIconView::SelectionMode)
    	CCONST_convert(VPROP(GB_INTEGER), -3, 
    		QIconView::NoSelection, SELECT_NONE, 
    		QIconView::Single, SELECT_SINGLE, 
    		QIconView::Multi, SELECT_MULTIPLE, 
    		QIconView::NoSelection));
...
BEGIN_PROPERTY(CICONVIEW_grid_w)
  ...
  if (READ_PROPERTY)
    GB.ReturnInteger(WIDGET->gridX() / MAIN_scale);
  else
  {
    WIDGET->setGridX(VPROP(GB_INTEGER) * MAIN_scale);
...
GB_DESC CIconViewDesc[] =
{
  ...
  GB_CONSTANT("_Properties", "s", "*,Mode{Select.*},Sorted,Arrangement{IconView.Free;Row;Column}=Row,GridWidth{Range:1;64}=8,GridHeight{Range:1;64}=8,WordWrap,Border=True,ScrollBar{Scroll.*}=Both"),
