...
#define IS_EXPAND(_object) CWIDGET_test_flag(_object, WF_EXPAND)
...
#define RAISE_ARRANGE_EVENT(_object) GB.Raise(_object, EVENT_Arrange, 0)
...
{
  //qDebug("MyContainer::resizeEvent %p", CWidget::get(this));
...
bool MyContainer::eventFilter(QObject *o, QEvent *e)
{
  ...
  if (type == QEvent::Move || type == QEvent::Resize || type == QEvent::Show || type == QEvent::Hide || type == EVENT_EXPAND)
  {
    ...
    //  qDebug("MyContainer::eventFilter: Resize");
    arrange_later(this);
  }
