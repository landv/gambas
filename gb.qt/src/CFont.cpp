...
#define FONT_STEP 20
#define GRADE_TO_SIZE(_grade) ((int)(powf(qApp->font().pointSizeFloat(), 1.0 + ((_grade) / (double)FONT_STEP)) + 0.5))
#define SIZE_TO_GRADE(_size)  ((int)(FONT_STEP * (logf(_size) / logf(qApp->font().pointSizeFloat())) + 0.5) - FONT_STEP)
  ...
  if (str.length())
  {
    ...
    for (QStringList::Iterator it = list.begin(); it != list.end(); ++it )
    {
      ...
      else if (flag[0] == '+' || flag[0] == '-' || flag[0] == '0')
      {
        ...
        f.setPointSizeFloat(GRADE_TO_SIZE(size));
      }
  ...
  if (READ_PROPERTY)
  {
    switch(prop)
    {
      ...
      case CFONT::Grade:
        ...
        GB.ReturnInteger(SIZE_TO_GRADE(f->pointSizeFloat()));
        break;
      ...
      case CFONT::Grade:
        {
          ...
          f->setPointSizeFloat(GRADE_TO_SIZE(g));
        }
        break;
