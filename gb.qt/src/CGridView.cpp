...
static void picture_property(void *_object, void *_param, bool create)
{
  ...
  {
    GB.StoreObject(PROP(GB_OBJECT), (void **)&item->data(create)->pict);
    if (create)
			WIDGET->updateCell(THIS->row, THIS->col);
	}
}
...
static void font_property(void *_object, void *_param, bool create)
{
  ...
  if (READ_PROPERTY)
  {
  	CFONT *font = item->data()->font;
  	
  	if (!font)
  	{
  		font = item->data()->font = CFONT_create(WIDGET->font(), CFONT_NORMAL);
  		GB.Ref(font);
		}
  ...
  {
    GB.StoreObject(PROP(GB_OBJECT), (void **)&item->data(create)->font);
    if (create)
			WIDGET->updateCell(THIS->row, THIS->col);
	}
}
