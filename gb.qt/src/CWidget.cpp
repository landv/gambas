...
void CWIDGET_new(QWidget *w, void *_object, char *klass /* = NULL */, bool no_filter /* = false */, bool no_tag /* = false */)
{
...
//#else
#define COORD(_c) ((WIDGET->isA("MyMainWindow")) ? ((CWINDOW *)_object)->_c : WIDGET->pos()._c())
...
static void set_color(CWIDGET *_object, int bg, int fg)
{
...
static int get_background(CWIDGET *_object)
{
...
static int get_foreground(CWIDGET *_object)
{
...
BEGIN_PROPERTY(CCONTROL_background)
  ...
  if (READ_PROPERTY)
    GB.ReturnInteger(get_background(THIS));
  else
  {
  	int col = VPROP(GB_INTEGER);
  	if (col != get_background(THIS))
  	{
			if (WIDGET->paletteBackgroundPixmap())
			{
				QPixmap p(*WIDGET->paletteBackgroundPixmap());
				set_color(THIS, col, get_foreground(THIS));
...
BEGIN_PROPERTY(CCONTROL_foreground)
  ...
  if (READ_PROPERTY)
    GB.ReturnInteger(get_foreground(THIS));
  else
  {
  	int col = VPROP(GB_INTEGER);
  	if (col != get_foreground(THIS))
  	{
			set_color(THIS, get_background(THIS), col);
    }
	}
...
static void post_gotfocus_event(void *control)
{
...
bool CWidget::eventFilter(QObject *widget, QEvent *event)
{
           ...
           || (type == QEvent::MouseMove))
  {
    ...
    if (event_id == EVENT_MouseMove && (state & QMouseEvent::MouseButtonMask)  && GB.CanRaise(control, EVENT_MouseDrag)
      ...
      CMOUSE_info.state = state;
			...
			GB.Raise(control, EVENT_MouseDrag, 0);
...
void CACTION_register(CWIDGET *control, const char *key)
{
	//qDebug("CACTION_register: (%s %p) %s %d", GB.GetClassName(control), control, key, on);
	...
	if (key && *key)
		CWIDGET_set_flag(control, WF_ACTION);
	else
		CWIDGET_clear_flag(control, WF_ACTION);
}

void CACTION_raise(void *_object)
{
	init_action();

	//qDebug("CACTION_raise: (%s %p)", GB.GetClassName(THIS), THIS);

	if (!CWIDGET_test_flag(THIS, WF_ACTION))
		return;
...
void CWIDGET_iconset(QIconSet &icon, QPixmap &pixmap)
{
	QImage img;
	QPixmap disabled;
	ulong *data;
	...
	icon = QIconSet(pixmap, QIconSet::Small);
