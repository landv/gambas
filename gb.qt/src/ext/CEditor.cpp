...
static QString purge(const QString & s)
{
  QString r;
  QChar c;
  uint i;
...
static void analyze(QString &s)
{
  ...
  GString gs = s;
  GDocument::highlightGambas(0, state, gs, &data, proc);
  s = gs.getString();
...
static void highlightCustom(GEditor *master, uint &state, GString &s, GHighlightArray *data, bool &proc)
{
...
BEGIN_METHOD(CEDITORDOC_purge_line, GB_INTEGER line)
...
GB_DESC CEditorDesc[] =
{
  ...
  GB_METHOD("GetPurgedLine", "s", CEDITORDOC_purge_line, "(Line)i"),
