...
void GDocument::insert(int y, int x, const GString & text)
{
  ...
  for(;;)
  {
    ...
    //qDebug("insert '%s' at %d:%d", text.mid(pos, pos2 - pos).latin1(), y, x);
...
int GDocument::wordLeft(int y, int x, bool word)
{
  ...
  for(;;)
  {
    if (xw == 0 || s.isWordChar(xw - 1) ^ word)
      break;
    xw--;
  }
...
int GDocument::wordRight(int y, int x, bool word)
{
  ...
  for(;;)
  {
    if (xw == (int)s.length() || s.isWordChar(xw) ^ word)
      break;
    xw++;
  }
...
void GDocument::highlightGambas(GEditor *, uint &state, GString &s, GHighlightArray *data, bool &proc)
{
  const char *src;
  EVAL_ANALYZE result;
  int i;
  ...
  if (y < 0)
	...
	while (y < numLines())
	{
		...
		if (l->s.length())
		{
			...
			(*highlightCallback)(views.first(), state, l->s, &l->highlight, proc);
			l->proc = proc;
