...
public:
	GArrayImpl() { GB.NewArray((void **)&array, sizeof(void *), 0); autoDelete = false; }
	~GArrayImpl() { GB.FreeArray((void **)&array); }

  uint count() const { return GB.Count(array); }
  void setAutoDelete(bool a) { autoDelete = a; }
  void *last() const { return count() ? array[count() - 1] : 0; }
  void append(const void *d) { *((const void **)GB.Add((void **)&array)) = d; }
  void clear();
  void *take();
