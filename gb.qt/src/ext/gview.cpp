...
static QColor calc_color(QColor ca, QColor cb, QColor cc, QColor cd)
{
	int r = 1, g = 1, b = 1, n = 0;
...
void GEditor::paintCell(QPainter * painter, int row, int)
{
  ...
  if (l->flag)
  {
		...
		if (l->flag & (1 << GLine::BreakpointFlag))
			b = styles[GLine::Breakpoint].color;
	}

  if (getFlag(ShowCurrentLine) && row == y)
    c = styles[GLine::Line].color;

	color = calc_color(a, b, c, styles[GLine::Background].color);
  cache->fill(color);
  ...
  if (getFlag(ShowProcedureLimits) && l->proc)
  {
  	if (getFlag(BlendedProcedureLimits))
  	{
  		...
  		p.drawTiledPixmap(0, 0, ur.width(), ur.height(), pattern);
		}
		else
		{
			QBrush brush(styles[GLine::Selection].color, Qt::Dense4Pattern);
  ...
  if (margin && (margin > ur.left()))
  {
    ...
    if (getFlag(ShowModifiedLines) && l->changed)
      p.fillRect(0, 0, margin - 1 - 4, cellHeight(), styles[GLine::Highlight].color);
    else if (getFlag(ShowLineNumbers))
      p.fillRect(0, 0, margin - 1 - 4, cellHeight(), styles[GLine::Line].color);
    ...
    if (getFlag(ShowLineNumbers))
    {
      ...
      if ((n % 10) == 0)
        ...
        p.setPen(styles[GLine::Selection].color);
      p.drawText(x1 + 4, fm.ascent(), QString::number(n).rightJustify(lineNumberLength));
    }
  }
...
void GEditor::cursorLeft(bool shift, bool ctrl)
{
  if (ctrl && x > 0)
  {
    ...
    nx = doc->wordLeft(y, x, false);
    if (nx > 0)
      nx = doc->wordLeft(y, nx, true);
...
void GEditor::cursorRight(bool shift, bool ctrl)
{
  if (ctrl && x < lineLength(y))
  {
    ...
    nx = doc->wordRight(y, x, true);
    if (nx < lineLength(y))
      nx = doc->wordRight(y, nx, false);
...
void GEditor::updateMargin()
{
  ...
  else if (getFlag(ShowModifiedLines))
    nm += 8;
