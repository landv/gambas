...
static int open_database(DB_DESC *desc, DB_DATABASE *db)
{
  ...
  else if ((db_fullpath = FindDatabase(name, (char *)conn->getHostName())) != NULL)
  {
...
{
  return do_query((SqliteDatabase *)db->handle, err, (Dataset **)result, query, 0);
}
...
{
  return do_query((SqliteDatabase *)db->handle, "Unable to begin transaction: &1", NULL, "BEGIN", 0);
}
...
{
  return do_query((SqliteDatabase *)db->handle, "Unable to commit transaction: &1", NULL, "COMMIT", 0);
}
...
{
  return do_query((SqliteDatabase *)db->handle, "Unable to rollback transaction: &1", NULL, "ROLLBACK", 0);
}
...
static int table_index(DB_DATABASE *db, char *table, DB_INFO *info)
{
  ...
  if (do_query(conn, "Unable to get primary index: &1", &res, qindex1, 1, table))
	  return TRUE;
  ...
  for (int i = 0; i < n; i++)
  {
    if (strstr(r->records[i][1].get_asString().data(), "autoindex") != NULL){
       ...
       if (do_query(conn, "Unable to get information on primary index: &1", &res, qindex2, 1, sql)){
           res->close();
           GB.FreeString(&sql);
           return TRUE;
       }
...
{
  return
    do_query((SqliteDatabase *)db->handle, "Unable to delete table: &1", NULL,
...
{
  return
    do_query((SqliteDatabase *)db->handle, "Unable to delete index: &1", NULL,
...
static int index_create(DB_DATABASE *db, char *table, char *index, DB_INDEX *info)
{
  ...
  return do_query((SqliteDatabase *)db->handle, "Cannot create index: &1", NULL, DB.Query.Get(), 0);
}
...
_CREATE_DATABASE:
  ...
  if (!do_query(&conn2, "Unable to initialise database", NULL, "CREATE TABLE GAMBAS (FIELD1 TEXT)", 0))
