...
void http_parse_header(CHTTPCLIENT *mythis)
{
	...
	GB.Alloc((void**)&mythis->ReturnString,(nposend+1)*sizeof(char));
...
int http_header_curl(void *buffer, size_t size, size_t nmemb, void *_object)
{
	...
	if (!THIS->len_header)
	{
		GB.Alloc((void**)&THIS->buf_header,sizeof(char*));
		GB.Alloc((void**)&THIS->buf_header[0],nmemb+1);
	}
	else
	{
		GB.Realloc((void**)&THIS->buf_header,sizeof(char*)*(1+THIS->len_header));
...
void http_initialize_curl_handle(void *_object)
{
	...
	{
		GB.Free((void**)&THIS->ReturnString);
...
BEGIN_METHOD_VOID(CHTTPCLIENT_free)
	...
	if (THIS->cookiesfile)	GB.Free((void**)&THIS->cookiesfile);
	if (THIS->ReturnString) GB.Free((void**)&THIS->ReturnString);
...
void http_reset(void *_object)
{
	...
	if (THIS->buf_header)
	{
		...
		GB.Free((void**)&THIS->buf_header);
		THIS->buf_header=NULL;
	}
	if (THIS->sContentType)
	{
		GB.Free((void**)&THIS->sContentType);
		THIS->sContentType=NULL;
	}
	if (THIS->sPostData)
	{
		GB.Free((void**)&THIS->sPostData);
		THIS->sPostData =NULL;
	}
	THIS->len_data=0;
	THIS->len_header=0;
